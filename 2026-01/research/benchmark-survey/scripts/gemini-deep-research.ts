 ...
 *
 * Usage: yarn gemini <question-id> <prompt-file-or-text>
 * Example: yarn gemini q1-calibration q1-calibration/prompt.md
 */
...
async function runDeepResearch(config: ResearchConfig): Promise<string> {
  ...
  if (!apiKey) {
    ...
    console.error('  "My Gemini key is AIza-xxx"');
    console.error('\nOr manually create ~/.claude/skills/deep-research/.env with:');
    console.error('  GEMINI_API_KEY=AIza-xxx\n');
    process.exit(1);
  }
